...
/**
    ...
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        // 1. 从请求头中获取令牌（前端需将token放在Authorization头中，格式：Bearer {token}）
        String authHeader = request.getHeader("Authorization");
        log.info("请求头中的Authorization：{}", authHeader);

        // 2. 校验令牌是否存在且格式正确
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            // 令牌不存在或格式错误，返回错误结果
            returnError(response, "请先登录，未携带有效令牌");
            return false;
        }
        ...
        try {
            ...
            // 6. 令牌解析失败（过期、签名错误等），返回错误结果
            log.error("令牌验证失败：{}", e.getMessage());
            returnError(response, "令牌无效或已过期，请重新登录");
            return false;
        }
    }
